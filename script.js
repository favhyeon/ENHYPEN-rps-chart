 ...
 * 멤버별 한 글자 표기(members 순서와 동일): 젱 젴 훈 선 양 닠
 */
const memberInitials = ["젱", "젴", "훈", "썬", "양", "닠"];

const pairNames = members.map((_, rowIndex) =>
    members.map((_, colIndex) =>
        `${memberInitials[rowIndex]}${memberInitials[colIndex]}`
    )
);
...
const CUSTOM_COLOR_KEY = "enhypen-custom-colors";
...
const STORAGE_KEY = "enhypen-yeop-rps";
...
saveBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const fileLabel = currentTab === "rps" ? "픈페스_취향표" : "공수_취향표";

        const link = document.createElement("a");
        link.href = currentBlobUrl;
        link.download = `ENHYPEN_${fileLabel}.png`;
